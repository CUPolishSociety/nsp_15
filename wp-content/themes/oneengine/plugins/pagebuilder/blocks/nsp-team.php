...
// CLEAR BLOCK
if( ! class_exists( 'OE_NSP_Team' ) ) :

class OE_NSP_Team extends AQ_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'NSP Team', 'oneengine'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('OE_NSP_Team', $block_options);
	}
	...
	function block($instance) {
		...
		?>
			...
			<div class="et-team-post item <?php echo $animation_effect ?>" <?php echo $duration_effect ?>>
            	<div class="team-info">
                	<h4 class="team-name" align="center"><?php the_title(); ?></h4>
                	
                </div>
                <div class="image-team-wrapper">
					<?php the_post_thumbnail( 'full', array('class' => 'et-post-thumbnail img-responsive') ); ?>
                </div>
                <div class="social-btn" data-name="<?php echo $post->post_title ?>" data-content="<h4 class='position'><?php echo get_post_meta( $post->ID, 'oe_team_position', true ); ?></h4> <?php echo $post->post_content ?>">Bio</div>
 	...
 	function before_block($instance) {
		extract($instance);
		echo 	'<div class="row"><div id="nsp_team_wrapper" class="col-md-10 col-md-offset-1 team-wrapper animation-wrapper">';
