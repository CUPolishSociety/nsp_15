...
// CLEAR BLOCK
if( ! class_exists( 'OE_CPartner_Block' ) ) :

class OE_CPartner_Block extends AQ_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'ET Corporate Partner', 'oneengine'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('OE_CPartner_Block', $block_options);
	}
	...
	function block($instance) {
		...
		?>
			...
			<div class="et-team-post height-width-auto item <?php echo $animation_effect ?>" <?php echo $duration_effect ?>>
            	<div class="image-team-wrapper" data-content="<?php echo do_shortcode(htmlspecialchars_decode($post->post_content)); ?>" data-title="<?php echo $post->post_title ?>">
					<span class="helper"></span><?php the_post_thumbnail( 'full', array('class' => 'et-post-thumbnail img-responsive') ); ?>
                </div>
			</div>
		<?php
			$i++;
 	...
 	function before_block($instance) {
		extract($instance);
		echo 	'<div id="corporate_partners_wrapper" class="team-wrapper animation-wrapper col-md-10 col-md-offset-1" >';
	...
	function after_block($instance) {
 		extract($instance);
 		echo 		'</div>';
 		echo 		'<div class="team-show-content"></div>';
 		echo   '</div><!-- END ET-BLOG-BLOCK-->';
	}

}

aq_register_block( 'OE_CPartner_Block' );
endif;
